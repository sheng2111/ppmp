...
import { useAuth } from "../context/AuthContext"; // ADAPT path if this page lives elsewhere
import {
...
// office, or fee category. "All Offices" always sits at the top so people
...
// The forEach callbacks below rely on the group's own fields — code,
// subtotal_quantity, subtotal_cost, q1_subtotal_qty, etc. — being available
// on the numbered type without retyping them.
...
// "Itemized List" — always scoped to the logged-in user's own office(s),
// regardless of role. Admins get the cross-office picker on a separate page
// (OfficesItemizedListReportPage, mounted at /admin/offices-itemized-list)
// instead of here.
export default function ItemizedListReportPage() {
  ...
  // A user's offices aren't stored on their account — offices are
  // per-PPMP (office_id + created_by), and one user can have created
  // PPMPs under more than one office. So instead of a single "home office"
  // field, this looks up every distinct office_id across THIS user's own
  // PPMPs. By default all of theirs are aggregated together; if they have
  // more than one, selectedMyOfficeId lets them narrow to just one (e.g.
  // "only DGTT/CTE, not BSCS/CITE") — still never anything outside their
  // own set.
  const [myOfficeIds, setMyOfficeIds] = useState<string[]>([]);
  const [myOfficeIdsLoaded, setMyOfficeIdsLoaded] = useState(false);
  const [selectedMyOfficeId, setSelectedMyOfficeId] = useState<string>(""); // "" = all of mine
  const myOffices = useMemo(
    () => flatOffices.filter((o) => myOfficeIds.includes(o.id)),
    [flatOffices, myOfficeIds],
  );
  ...
  // Scoped to only the offices this user's own PPMPs actually use — by
  // default aggregated together, or narrowed to one specific office of
  // theirs via selectedMyOfficeId.
  const effectiveOfficeId = selectedMyOfficeId || undefined;
  const effectiveOfficeIds = selectedMyOfficeId
    ? undefined // narrowed to one of their own offices — no need to aggregate
    : myOfficeIds.length > 0
      ? myOfficeIds
      : undefined;

  // Office tree — needed just to resolve this user's own office ids into
  // display names. Same endpoint CreatePPMPPage.tsx already uses
  // successfully.
  useEffect(() => {
    api
  ...
  // Find every office_id across PPMPs THIS user created, via the existing
  // created_by filter on GET /ppmps/ — no new backend endpoint needed for
  // this.
  useEffect(() => {
    if (!dbUser?.id) return;
    api
      .get("/ppmps/", { params: { created_by: dbUser.id } })
      .then((res) => {
        const ids = Array.from(
          new Set(
            (res.data as { office_id?: string | null }[])
              .map((p) => p.office_id)
      ...
      .catch(() => setMyOfficeIds([]))
      .finally(() => setMyOfficeIdsLoaded(true));
  }, [dbUser?.id]);
  ...
  // Wait for the own-office lookup to finish first — otherwise the very
  // first request would go out with no office filter at all and briefly
  // show everyone's data.
  useEffect(() => {
    if (myOfficeIdsLoaded) {
      runReport();
    }
  ...
  return (
    ...
    >
      <PageHeader
        title="Itemized Procurement List"
        subtitle="Auto-generated from approved PPMPs — quarterly quantities are pulled from what was already entered, never re-entered here."
      />
      ...
      <div className={`${T.card} p-5 mb-6 flex flex-wrap items-end gap-4`}>
        ...
        <div className="w-64 max-w-full">
          ...
          {!myOfficeIdsLoaded ? (
            ...
            // Used more than one office (e.g. DGTT/CTE and BSCS/CITE) —
            // let them narrow to just one instead of always seeing both
            // aggregated. Search is scoped to only their own offices;
            // there's nothing outside that set to pick from.
            <OfficeSearchPicker
              value={selectedMyOfficeId}
              flatOffices={myOffices}
              onChange={setSelectedMyOfficeId}
              allLabel="All My Offices"
            />
          ) : (
            // Only ever used one office (or none yet) — nothing to narrow,
            // so just a read-only display instead of a picker with one option.
            <div
              className={`${inputClass} bg-[#F8FAFC] text-[#64748B] cursor-not-allowed truncate`}
            >
              {myOffices.length === 0 ? "No office yet" : myOffices[0].name}
            </div>
          )}
        </div>
